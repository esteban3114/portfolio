/* global React, ReactDOM, useTweaks, TweaksPanel, TweakSection, TweakSlider, TweakToggle, TweakRadio, TweakButton */
const { useState, useEffect, useRef, useCallback } = React;

/* ============================================================
   EDITABLE PROFILE DATA  — replace placeholders as needed
   ============================================================ */
const PROFILE = {
  ticker: "ESTBN",
  name: "Esteban",
  age: "18",
  location: "France",            // not specified in brief — placeholder
  status: "Student / Developer",
  focus: "Python · Automation · Trading bots",
  github: "github.com/esteban",  // placeholder username
  email: "esteban@example.com",  // placeholder email
};

/* ---- helpers to build colored, aligned terminal lines ---- */
// a "line" is an array of segments: { t: text, c: className }
const seg = (t, c = "label") => ({ t, c });
const pad = (s, n) => (s + " ".repeat(Math.max(0, n - s.length)));
// numbered data row, Bloomberg style:  "11) LABEL............ VALUE"
function row(num, label, value, vclass = "val") {
  return [
    seg(num + ") ", "num"),
    seg(pad(label, 20), "label"),
    seg(value, vclass),
  ];
}
...
const SECTIONS = {
  ...
  CONTACT: {
    ...
    lines: () => [
      ...
      blank(),
      [seg("> ", "num"), seg("Type ", "label"), seg("1", "num"), seg("..", "label"), seg("4", "num"), seg(" or click a function to navigate.", "label")],
      [seg("> ", "num"), seg("Press ", "label"), seg("GO", "valbold"), seg(" to send a message.", "label")],
    ],
  },
};
...
/* news ticker lines (dev-flavored, Bloomberg news style) */
const NEWS = [
...
/* ============================================================
   TYPEWRITER  — reveals colored lines character-by-character
   ============================================================ */
function useTypewriter(lines, speedMs, deps) {
  const total = lines.reduce((a, l) => a + l.reduce((b, s) => b + s.t.length, 0) + 1, 0);
  const [count, setCount] = useState(0);
  useEffect(() => { setCount(0); }, deps); // reset on section change
  useEffect(() => {
    if (count >= total) return;
    // type a few chars per tick for snappiness
...
function TypedLines({ lines, count }) {
  // walk segments, revealing up to `count` characters; place cursor at frontier
  let remaining = count;
  const out = [];
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    // newline accounts for 1 char
    const isFrontier = remaining > 0 && remaining <= 1 && !cursorPlaced;
    if (remaining <= 0 && !cursorPlaced) {
      // cursor sits at end of last revealed line
    }
    remaining -= 1;
...
function Boot({ onDone, speed }) {
  ...
  const [chars, setChars] = useState(0);     // chars in current line
...
function App() {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (i >= 1 && i <= ORDER.length) setActive(ORDER[i - 1]);
      if (e.key === "ArrowRight") setActive((c) => ORDER[(ORDER.indexOf(c) + 1) % ORDER.length]);
      if (e.key === "ArrowLeft") setActive((c) => ORDER[(ORDER.indexOf(c) - 1 + ORDER.length) % ORDER.length]);
    };
    window.addEventListener("keydown", onKey);
  ...
  return (
    <div className={"app " + (t.scanlines ? "scanlines" : "")} style={rootStyle}>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakButton label="Replay boot sequence" onClick={() => { sessionStorage.removeItem("estbn_booted"); setBooted(false); }} />
      </TweaksPanel>
    </div>
  );
}
